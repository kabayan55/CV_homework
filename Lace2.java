	...
	static MyImage execute(MyImage input1, MyImage input2, MyImage input0) { 
		...
		for(int i = 0; i < height; i++) {
			for(int j = 0; j < width; j++) {
				...
				if(i < height1 && width1*2 < j) {

			       
				    Color color0 = input0.getColor(j-width1*2, i);
					if(color0.getRed() > 10) {
				
					    if(i<height2 && j<width2){
						Color color1 = input1.getColor(j-width1*2, i);
						...
						Color color1 = input1.getColor(j-width1*2, i);			                                           output.setColor(j, i, color1);
					    }
						isProcessed = true;
					}
